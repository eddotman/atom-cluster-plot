...
def compute_grid (grid_size, grid_file, start_row):
	...
	mlab.contour3d(grid3d, "3D Density Isosurface Map", transparent = True)
	mlab.show()
	mlab.surf(grid2d, warp_scale = 0.05)
	mlab.show()
	
#Compute coordination number from g(r)
def compute_coord_num (rho, lb, ub):
...
def compute_clusters (cfg_file, cfg_start_row, box_size, num_atoms, cluster_coord=4, nn_file = "nn_list", box_length=62.024362):
	...
	sing_clusters = empty((0,cluster_coord))
...
def analyze_clusters (cluster_num = 8):
	...
	return (dih_atoms, dih_angles, xz_angles, z_dists, atom2_dists, plane_angles)
